"""
btc_price_daily — Daily BTC/USD price ingestion from CoinGecko.

Flow:
  fetch_and_load_btc_price  →  transform_to_model  →  validate_load

Ingests the daily price snapshot from the CoinGecko simple-price endpoint,
writes to db_stage.btc_price_daily, transforms into
db_data_model.fact_btc_daily_price, and validates a row exists.
"""
...
STAGE_TABLE = "db_stage.btc_price_daily"
MODEL_TABLE = "db_data_model.fact_btc_daily_price"
...
def _fetch_and_load_btc_price(**context):
    price_date = _get_price_date(context)
    log.info("Fetching BTC price from CoinGecko for price_date=%s", price_date)
    ...
    if "bitcoin" not in payload or "usd" not in payload["bitcoin"]:
        raise ValueError(
            f"CoinGecko response missing required key 'bitcoin.usd'. Raw: {raw!r}"
        )
    ...
    price_change_24h = btc.get("usd_24h_change")
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO db_stage.btc_price_daily (
                    price_date,
                    price_usd,
                    market_cap_usd,
                    volume_24h_usd,
...
def _transform_to_model(**context):
    price_date = context["ti"].xcom_pull(
        task_ids="fetch_and_load_btc_price", key="price_date"
    )
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO db_data_model.fact_btc_daily_price (
                    price_date,
                    price_usd,
                    market_cap_usd,
                    volume_24h_usd,
...
def _validate_load(**context):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT COUNT(*) FROM db_data_model.fact_btc_daily_price WHERE price_date = %s",
                (price_date,),
            )
    ...
    if count == 0:
        raise ValueError(
            f"No rows found in fact_btc_daily_price for price_date={price_date}. "
...
with DAG(
    ...
    fetch_and_load_btc_price = PythonOperator(
        task_id="fetch_and_load_btc_price",
        python_callable=_fetch_and_load_btc_price,
    )
    ...
    start >> fetch_and_load_btc_price >> transform_to_model >> validate_load >> end
